...
**배경 사진 레이어링은 `object-fit:cover` 가 아니라 Figma 원본 방식(퍼센트 트랜스폼)을
그대로 씁니다.** Figma 의 Banner Area(`5749:63752` 모바일 / `5762:66430` 데스크톱)는
`<img>` 를 `height:163.04%; top:-5.49%` 식으로 컨테이너보다 키운 뒤 `overflow:hidden` 으로
잘라내는 방식을 씁니다 — object-fit 한 줄로 뭉치지 않고, 배경 그라디언트·사진·딤을 각각
독립된 레이어로 쌓아 올리는 구조입니다. 지금 쓰는 사진(1168×1106)에 맞춰 같은 방식으로
`.sb-bg img{width:100%;height:151.7%}`(모바일) / `height:210.4%`(데스크톱, 1200px↑)
로 계산해 넣었습니다 — 두 값 모두 위쪽 기준(top:0)이라 결과 화면은 이전 `object-fit:cover;
object-position:50% 0` 과 동일하지만, Figma 가 실제로 쓰는 레이어 구조와 정확히 같은
방식입니다.
