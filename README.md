...
| 파일 | 캡처한 노드 | 용도 |
|---|---|---|
| `assets/banner/livemd_장한음.jpg` | `5766:73166` (Banner Area, 텍스트 없는 배경 전용 레이어) | LIVE MD 쇼케이스 배너 배경 사진 |
...
처음엔 `Banner Area`(`5749:63752`, 텍스트까지 포함된 완성 프레임)를 그대로 스크린샷해서
썼다가, 그 프레임의 자식 레이어("Meet Your Artist Event!" 타이틀·타이머 숫자 등)가 그림
위에 이미 구워져 나와 우리 쪽 실시간 HTML 텍스트와 겹쳐 보이는 문제가 있었습니다. 이후
`5766:73166`(같은 배경을 텍스트 없이 따로 분리해 둔 참고 레이어)을 알려주셔서 깨끗한
배경 사진으로 교체했습니다.

딤(스크림) 처리는 같은 참고 섹션의 `Dim`(`5766:73288`, 375×234 단색 오버레이) 레이어에서
실제 픽셀을 샘플링해 `linear-gradient(180deg, rgb(18,18,18) 0%, rgb(54,54,54) 100%)` 로
맞췄습니다 — 토큰 이름(`black80→mask40`)만으로 추정했던 이전 값보다 실물에 더 가깝습니다.
